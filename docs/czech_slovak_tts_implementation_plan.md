...
Normalizer tests (tested, 94 tests; the first run downloads the Stanza models, 250 MB):
...
```bash
uv run pytest tests/ -q -m "not slow"   # 293 tests, ~15 s on CPU

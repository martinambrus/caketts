...
Normalizer tests (tested, 112 tests; the first run downloads the Stanza models, 250 MB):
...
```bash
uv run pytest tests/ -q -m "not slow"   # 311 tests, ~15 s on CPU

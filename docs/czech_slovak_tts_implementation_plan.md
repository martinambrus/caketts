...
Normalizer tests (tested, 90 tests; the first run downloads the Stanza models, 250 MB):
...
```bash
uv run pytest tests/ -q -m "not slow"   # 289 tests, ~15 s on CPU
